...
class TagPageOpen extends React.PureComponent<IProps, IState> {
    ...
    componentDidUpdate() {
        setTimeout(() => {
            this.moveToView(
                this._currentTag as HTMLDivElement,
                this._scrollViewWrapper as HTMLDivElement
            )
        }, 100)
    }
    ...
    moveToView(currentTag: HTMLDivElement, scrollView: HTMLDivElement) {
        ...
        if (currentTag.offsetLeft < -scrollBodyLeft) {
            ...
            currentTag.offsetLeft + currentTag.offsetWidth <
                -scrollBodyLeft + scrollView.offsetWidth - 80
        ) {
            // 标签在可视区域
            this.setState({
                scrollBodyLeft: Math.min(
                    0,
                    scrollView.offsetWidth -
                        100 -
